...
local stabilization = shared.Forbidden.PointAssist.Stabilization
local smoothingFactor = stabilization -- Base smoothing value from shared settings
local targetCFrame = DesiredCFrame
        ...
        if BodyPart then
            ...
            if predictedPosition then
                ...
                if shared.Forbidden.PointAssist.StabilizationEnabled then
                    -- Apply dynamic smoothing with deceleration effect for more natural motion
                    local distance = (targetCFrame.Position - currentCFrame.Position).Magnitude
                    local dynamicSmoothing = smoothingFactor * (1 - math.min(distance / 50, 1)) -- Decelerates the closer you are to the target
                    
                    -- Introduce a small random variation to make it more human-like
                    local randomVariation = math.random() * 0.02 - 0.01 -- Randomness between -0.01 and 0.01
                    dynamicSmoothing = dynamicSmoothing + randomVariation
                    
                    -- Lerp with the adjusted smoothing
                    Camera.CFrame = currentCFrame:Lerp(targetCFrame, dynamicSmoothing)
                else
